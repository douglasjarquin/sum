...
Only when the boss confirms the old coordinator pane is gone, run `./bin/sumctl init --role coordinator --reclaim`. It refuses while the old pane runs an agent or while Herdr cannot observe it, and it never rebinds tasks by itself.
