...
1. Stage the target's next immutable revision from that runtime: for a task, `brief regenerate` (`briefs/rN.md`, machine-generated change summary, decisions as recorded); for the coordinator, a contract snapshot `.sum/coordinator/contracts/rN.md` rendered from the runtime's `AGENTS.md` bootstrap followed by its coordinator core `COORDINATOR.md` (a runtime without that core stages nothing and the request fails naming it), with the skills' hashes in its policy. Unchanged content stages nothing.
