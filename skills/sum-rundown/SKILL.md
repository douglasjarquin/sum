...
A `cleanup: pending` row means the exact PR was observed merged and the task still holds its workspace and slot; offer `sumctl cleanup TASK_ID` (see `skills/sum-delivery/SKILL.md`). A `blocked` row names the blocker codes; relay them, do not clear them by force.
