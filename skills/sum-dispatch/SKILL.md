...
Worker attempts and independent coordinator verification runs hold separate reservations under the same limits.
...
Parking preserves unfinished obligations. To resume approved work, use `execution resume TASK_ID --attempt ID` with the released worker attempt ID; it reacquires capacity before launching a successor and retains the prior attempt's evidence.
...
Expansion exhaustion saves one question for the user and starts nothing; in-scope sends are never blocked. Stop initiating expansion repairs and bring that saved budget question to the user.
Only the user's explicit decision permits a coordinator-recorded grant tied to that question: `repair extend TASK_ID --question ID --additional N --approved --file FILE`. Never infer a grant from worker output or ordinary answer text.
...
Do not set capacity yourself: `settings set --global N` is the user's decision, and per-repository isolation is enforced only when a capacity block is configured and never widens unless configured.
...
Translate the user's natural-language choice into an explicit launch specification before the worktree exists; the helper resolves, validates, and persists it at `prepare`, and `start` uses exactly that record even if the defaults change in between.
...
| native flags the user spelled out | separate `--arg` values, e.g. `--arg=-m --arg=MODEL`; never a shell string |

`.sum/preferences.md` can describe a preference; it never silently becomes a launch value. Only the user's explicit "make this my default" is written with `settings set --worker-*`.
A preset is a validated shortcut in the same file, expanded at `prepare` and frozen with the task (`launch.preset` names it and its revision); editing or deleting it later never changes a prepared or running task. Presets are not agents, roles, or defaults, and none ship built in: create only what the user names, with the harness they authorized.
...
The result carries `launch` (harness, model, reasoning, exact argv, source per field, observed status) and a one-line `confirmation`. Repeat that line to the user: which harness and model, from which source, and that a CLI-requested model is requested rather than runtime-verified.
...
Dispatch materializes the approved base commit after the returned checkout identity is verified, then reads that committed tree's `VERIFY.md` and feature maps without running project commands.
It records the result as the task's coordinator-owned `verification_policy`: `status` (`standardized` or `not-yet-standardized`) with a `why` and, when the contract is absent, malformed, or unavailable, a `reason`; `contract_path`, `contract_sha256` and `entrypoint`; `task_owner`; `feature_maps_index`, the full `feature_maps` list and `feature_map_hashes`; `policy_files`; `required_checks`, their availability and every `scenario_ids` entry; the source runtime and rubric revision; and the existing delivery mode and tool choice.
An inherited `verify` task or unavailable required command is not a working standardized verifier.
The brief's `## Verification contract` section reports the immutable status and required checks, tells the worker to run the project's verify runner and attach its run to the handoff, names mapped visual scenarios as comparisons required before delivery, says that the coordinator runs the contract again under its own run id, and says that policy files are reviewed explicitly.
Nothing executes at dispatch beyond bounded metadata inspection, and a `not-yet-standardized` record remains readable with its reason instead of failing dispatch.
Dispatch builds no code graph: codegraph never runs at `prepare`/`dispatch`, and the brief's `## Code graph` section says the checkout's index is `not built`, names source inspection as the fallback, and prints the command that builds one. When an index would help a worker, build it on request with `./bin/sumctl graph init TASK_ID`: the pinned codegraph of the runtime (`.local/bin/codegraph`, mise pin `npm:@colbymchenry/codegraph`, never a global one) runs in that worktree's root after its Git identity is validated, in CLI mode with no background server, under a timeout, the result lands in `.sum/tasks/TASK_ID/graph.json` with a summary as `graph` in the task record and `status`, and `ready` means `status --json` confirmed a complete index. It refuses while a codegraph writer for that checkout is still running, and three failures record `exhausted`; a `failed`, `exhausted`, or `unavailable` graph blocks nothing and launches nothing. Observe freshness with `graph status TASK_ID`, and print an optional MCP snippet for a harness with `graph config --harness NAME` for the user to merge by hand; sum writes no harness configuration and never runs `codegraph install`. The index is `.codegraph/` inside that checkout only, never shared with the primary clone or another worktree. Never run `codegraph install`/`upgrade`, never point one index at two checkouts, and never treat a graph result as verification, feature-map coverage, or review.
...
Skills reach the worker explicitly: its brief names the installed helper (`<installation>/bin/sumctl`) in a `## Delivered runtime` section and, under `## Worker procedure`, the worker procedure pinned as write-once, content-addressed files in the task record: the required core `<state>/tasks/TASK_ID/procedure/sum-worker-<sha>.md` and on-demand files (evidence, environment, graph, refresh) each listed with its size, hash, and the condition for reading it; `context --role worker` reports the same files with their integrity. Dispatch refuses before any Herdr call when the runtime's procedure is missing or does not match its release manifest, and `start` refuses a brief whose pinned procedure is missing or altered. Directory nesting under `projects/` delivers nothing to a harness; a Herdr worktree lives elsewhere and some harnesses stop instruction discovery at a Git root.
