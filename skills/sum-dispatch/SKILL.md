...
Dispatch reads the checkout's `VERIFY.md` and feature maps the way the verify runner parses them and records the result as the task's `verification_policy`: `status` (`standardized` or `not-yet-standardized`) with a `why` and, when the contract itself is absent or malformed, a `reason`; `contract_path`, `contract_sha256` and `entrypoint`; `task_owner`; `feature_maps_index` and the full `feature_maps` list; `policy_files`; and `evidence_required`, one `{scenario, feature, map}` row per mapped scenario whose Evidence cell names visual proof. The brief's `## Verification contract` section reports that status, tells the worker to run the project's verify runner and attach its run to the handoff, names those scenarios as comparisons required before delivery, says that the coordinator runs the contract again under its own run id, and that policy files are reviewed explicitly. Nothing is executed at dispatch, and a contract sum cannot read records `not-yet-standardized` with the reason instead of failing the dispatch.
