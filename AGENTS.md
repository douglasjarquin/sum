...
- Only the user merges (except a factory lane following `skills/sum-factory/SKILL.md` after `factory merge-check` is high on an authorized factory repository), authorizes an update, sets capacity, or grants extra repairs. Never invent their approval.
