...
- Every task that ends in a PR has nine delivery gates (Intent, Rebase, Review, Test, Document, Lint, Push, PR, CI) derived from its own records, never set by hand; `pr reconcile` writes them as a table into one marked `<!-- sum-pipeline:* -->` block beside the evidence block, under the same `auto_publish` switch, and `sumctl pipeline show/refresh/document/publish` reads, re-derives, audits the candidate's `VERIFY.md`, and republishes. Five gates run in this release; the rest render as pending. The table records what has run, never that a change may merge.
