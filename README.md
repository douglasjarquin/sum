...
| Delivery pipeline in the PR | Nine ordered gates (Intent, Rebase, Review, Test, Document, Lint, Push, PR, CI) derived from the task's own records and rendered as a status table in one marked `<!-- sum-pipeline:* -->` block of the reconciled PR, beside the evidence block. Intent, Review, Test, Document, and PR run in this release; the rest render as pending. `sumctl pipeline show/refresh/document/publish`; `pr reconcile` publishes it under the same `auto_publish` switch as the evidence block. The table records what has run, never that a change may merge |
