...
| Delivery pipeline in the PR | Nine ordered gates (Intent, Rebase, Review, Test, Document, Lint, Push, PR, CI) derived from the task's own records and rendered as a status table in one marked `<!-- sum-pipeline:* -->` block of the reconciled PR, beside the evidence block. Eight run in this release and only CI renders as pending: Rebase observes the candidate against its base branch without ever rewriting it, Lint runs whatever lint task the project itself declares, and Push fast-forward pushes the reported candidate and nothing else. `sumctl pipeline run` drives the coordinator's gates in order and stops at the first failure; `show/refresh/rebase/lint/push/document/publish` do them one at a time, and `pr reconcile` publishes the table under the same `auto_publish` switch as the evidence block. The table records what has run, never that a change may merge |
...
| Allowlisted LSP binaries | `pipx:basedpyright` 1.40.1 and `go:golang.org/x/tools/gopls` 0.23.0 from mise; `mise run setup` links them into `.local/bin` once; project Grok and Cursor PostToolUse hooks run `sumctl lsp ensure` for those missing binaries only; existing links are never rewritten |
