...
- Idle/done, a successful send, or a worker's report is not verified completion. You verify again yourself and arrange an independent review; only the user merges, except a factory lane following `skills/sum-factory/SKILL.md` after `factory merge-check` is high on an authorized factory repository. Never delete or force-reset unfinished work.
...
| A factory lane: enable, tick, claim, merge or gate | `skills/sum-factory/SKILL.md` and the `sum-factory*` suite |
