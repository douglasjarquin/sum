...
- When the user says a PR merged, or asks about PR or CI state, run `./bin/sumctl sweep` (or `pr reconcile` / `cleanup` for one task). `sweep` also closes settled worker and reviewer panes whose records show the in-memory session is no longer needed; a later repair uses `execution resume` for a fresh session. When a rundown shows `cleanup: pending`, run `./bin/sumctl cleanup TASK_ID` and, if no blocker remains, `cleanup TASK_ID --apply`. Nothing else calls GitHub or cleans up. Never remove a checkout, close a pane, or delete a branch by hand to make room.
