...
- When the user says a PR merged, or asks about PR or CI state, run `./bin/sumctl sweep` (or `pr reconcile` / `cleanup` for one task). Nothing else calls GitHub or cleans up. Never remove a checkout, close a pane, or delete a branch by hand to make room.
