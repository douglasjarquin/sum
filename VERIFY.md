...
Scoped tasks stay available for iteration: `mise run test` (suites only), `mise run lint` (`gofmt -l` and `go vet` over `go/`, plus basedpyright on the portable skill scripts), and `mise run demo`.
