...
| Bundled skills | Dispatch, worker execution, verification/PR delivery, rundown/recovery, isolated self-development, atomic updates with rolling session refresh and code-only rollback, and the optional per-project factory lane |
...
| Delivery pipeline in the PR | Nine ordered gates (Intent, Rebase, Review, Test, Document, Lint, Push, PR, CI) derived from the task's own records and rendered as a status table in one marked `<!-- sum-pipeline:* -->` block of the reconciled PR, beside the evidence block. All nine run: Rebase observes the candidate against its base branch without ever rewriting it, Lint runs a checkout-declared dependency bootstrap when one exists, then whatever lint task the project itself declares, Push fast-forward pushes the reported candidate and nothing else, and CI reads the checks GitHub reports on the PR. CI is observed, never watched, because sum runs no daemon and polls nothing: the checks are read at `pr reconcile` and on demand with `sumctl pipeline ci`, and the row is stamped with when the checks last changed, so a green row means green as of the time shown and a re-read that finds nothing moved edits nothing. A feature-map row whose Evidence cell names a screenshot, screencast, or red/green pair blocks Test until a comparison for that candidate exists, so user-visible work cannot reach origin unproven; the worker captures it with `.agents/skills/evidence/`, or the user waives it and the coordinator records that decision with `verify --accept-missing-evidence`. `sumctl pipeline run` drives the coordinator's gates in order through opening the PR and reconciling it, and stops at the first failure or a blocked Test: it reads GitHub for a pull request on the branch first, so a second run creates nothing, and `--no-pr` stops after Push. `sumctl pipeline show` and the matching refresh, rebase, lint, push, document, pr, ci, and publish commands do the gates one at a time, and `pr reconcile` publishes the table under the same `auto_publish` switch as the evidence block. The table records what has run, never that a change may merge |
...
Every recorded pane address is judged against the occupant sum bound it to before it grants a role or submits anything. One comparator in `go/internal/incarnation` covers `init` role recognition, coordinator commands, the bridge and MCP, the delivery claim, `repair send`, and refresh. It compares the Herdr `terminal_id`, native agent session, and shell process identity recorded when the pane was bound. In the delivery pass, the fresh `agent get` taken right before the prompt is judged inside the state-locked claim against re-read records. A shell probe it needs is taken before the lock, and a refused occupant gets nothing sent and nothing recorded. The outcomes and recovery are in `docs/recovery.md` (Pane incarnation).

A pass has one budget (default 20 s, `pump --budget SECONDS`; one hook event shares one budget across its pumps). Delivery is serialized per recipient, not per installation: each recipient pane (its canonical machine, session, and pane, so a legacy hostname spelling of this host names the same pane) has its own lock in a deliver directory under the state home, and two operations delivering to different recipients run in parallel while two delivering to one recipient take turns. A recipient another operation is delivering to is set aside after one try and revisited after every other recipient, waiting only as long as an observation and a prompt could still follow within the budget. Lock order is fixed and enforced (an out-of-order acquisition fails instead of deadlocking): the delivery compatibility lock `.deliver.lock`, then one recipient lock, then the state lock `.lock`. Deliveries of this release hold `.deliver.lock` shared; releases before recipient-scoped delivery hold it exclusively, so mixed versions still serialize completely. The state lock guards record writes only and is never held across a Herdr call. With the recipient lock held, each task is re-read and its route and open obligations recomputed, so an obligation closed or a route rebound since the pass read the task gets nothing. Right before the prompt, under the state lock, the pass re-reads once more, drops any return that closed or was rebound while the recipient was observed and rebuilds the notice from the rest (when none remain the recipient is `quiet` and nothing is recorded), confirms the prompt still fits the budget, and records the attempt `in-flight`; the outcome is recorded under the state lock afterwards. One `agent list` per Herdr session selects recipients (busy, absent, or wrong-checkout recipients are `not-delivered` without their own call), a settled recipient is re-observed with `agent get` right before `agent prompt`, and a Herdr call starts only when its own timeout plus the runner's pipe grace still fits the budget, so no started call is cut and no prompt turns `uncertain` because of the budget. A failed, timed-out, or unknown-effect call makes its session unavailable for the rest of the pass. Recipients the budget or a busy lock did not reach are `deferred`: no delivery record, still pending, visited first next time, since the caller's own inline listing goes first and then the least recently attempted recipients. A pending return is sent once; a known failure (busy, absent, wrong checkout, not registered) is retried on later passes up to three times and then shows `stalled`; a timeout after a possible submission or an interrupted pass stays `uncertain` and is never re-sent by itself. `sumctl notice TASK_ID --to parent|worker` is the explicit single retry for those. `show` and `inbox` carry the `returns` view. The returns sidecar is the only persisted record of an attempt; no pass rewrites a task record. The single `notice` object commands still print is derived from the task's latest attempt at read time, and a task record's own `notice` field is history an older release wrote, shown only when the sidecar has no attempt for that task. After a rollback, an older release shows that record field, which may be null or out of date for returns this release delivered, until it records its own attempt; it decides delivery from the sidecar, never from that field.
