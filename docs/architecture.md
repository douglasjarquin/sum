...
Every pending return is derived from the records themselves: an open question, an unverified report, and a recorded review verdict are owed to the parent, an unapplied answer and a requested brief revision to the worker. Nothing has to be acknowledged for it to stay listed, and nothing but a later record (an answer, `resolve`, `answer --close`, `verify`, a PR observation, `brief adopt`) closes it. A review return closes when the coordinator acts on the verdict for that candidate (its own `verify`, `pipeline push`, `pipeline pr`, `pipeline ci`, or a PR publication, or a `repair send` after the verdict) or when a later report, handoff, or review names a newer candidate. The per-task returns sidecar keeps only notification state, so a legacy helper rewriting the task record cannot erase it.
