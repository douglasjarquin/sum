...
- **Approach:** `Source{Name, Path, Load, When}`, `var Sources`; `Check(runtimeRoot) error` (preflight, no writes); `Pin(runtimeRoot, taskDir) ([]any, error)` returning ordered rows; `Verify(taskDir, rows) error`; `Rows(policy)` reader; `Sha(rows, name)`. Release manifest comparison reads `files[source]` (`sha256:<hex>`) from `release.json` when present. Errors name the source path, the runtime, and the recovery (restore the tracked file with `git checkout` in a checkout runtime, or activate a verified release), and say no brief was written.
