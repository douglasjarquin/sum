...
The worker learns nothing from directory nesting. Its brief's `## Delivered runtime` section names the installed helper (`<installation>/bin/sumctl`), and its `## Worker procedure` section names the worker procedure pinned as write-once files in the task record with size and hash; `context --role worker` reports the same `helper`, the runtime skill `files`, and the pinned `procedure` rows with their integrity. A Herdr worktree lives outside the installation, some harnesses stop instruction discovery at a Git root, and a project's own `AGENTS.md` and mise tasks remain the project's.
