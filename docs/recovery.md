...
Decision priority attempts (an open question sent behind an outstanding routine episode) follow the per-obligation attempt rules, not the episode's: a `not-delivered` attempt (busy or absent pane) leaves the decision unrecorded and the next eligible pass tries once more; a `submitted` or `uncertain` attempt records the decision on the sidecar and it is never resent by a pass (an uncertain one included); `wake show` lists recorded decisions with their deliveries under `priority_prompts`, so they are never reported as uncoalesced. `wake reconcile` leaves them alone except when it closes an episode as `replaced`, which clears them: a new occupant inherits nothing and is prompted once. A decision leaves the record only when its question is no longer open.
