...
| `packaging.native-companion` | Setup and immutable release staging build a cgo-free native companion beside the existing Python/Node runtime, record dependency provenance, and reuse installed artifacts without replacing them | automated: `tests/test_packaging.py`, `tests/test_native_packaging.py`, `tests/test_core.py` | `docs/dependency-inventory.json`, `docs/DEPENDENCIES.md`, `scripts/setup.py`, `lib/sumctl.py`, `mise.toml`, and the staged release manifest |
