...
class VerifyLab(unittest.TestCase):
    ...
    def test_sum_repository_contract_checks_against_its_own_tasks(self):
        ...
        self.assertEqual([i for i, d in drivers.items() if d == "manual"], ["live.herdr-smoke", "live.harness-canary", "live.grok-bot-canary", "verify.sum-self", "ci.hosted-verification", "root.real-harness-canary", "evidence.publish-rendered", "evidence.sum-self", "graph.real-binary", "graph.harness-mcp", "skills.review-workflow"])
