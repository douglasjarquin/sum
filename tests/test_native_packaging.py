...
class NativePackagingTest(unittest.TestCase):
    def test_native_build_produces_only_required_mesh_binary_without_runtime_tools(self):
        with tempfile.TemporaryDirectory(prefix="sum-native-build-") as name:
            ...
            with mock.patch.dict(sumctl.os.environ, {"SUM_GO_BIN": shutil.which("go"), "GOROOT": "/stale/go", "GOBIN": "/stale/go/bin", "GOTOOLDIR": "/stale/go/pkg/tool", "GOTOOLCHAIN": "local", "GOOS": "linux", "GOARCH": "amd64"}):
            ...
            self.assertEqual(output, target / ".local" / "bin" / "herdr-mesh-go")
            self.assertFalse((target / ".local" / "bin" / "sumctl-go").exists())
            self.assertEqual(sumctl.native_platform(), expected)

    def test_native_build_does_not_replace_existing_mesh_executable(self):
